...
api.post('/signup', async (req, res) => {
  ...
  const { firstname, lastname, email, password, passwordConfirmation, gender, telenumber, birthdate, zipCode, city, adress } = req.body
...
api.put('/reset-password', passport.authenticate('jwt', { session: false }), async (request, response) => {
  const { id } = request.user
  const { oldPassword, newPassword, confirmationPassword } = request.body
